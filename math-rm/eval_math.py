...
def process_results(doc, completion, answer):
    split_ans = completion.lower().split('the answer is:')
    if len(split_ans) > 1:
        ans = split_ans[-1].strip().strip('ки\n').strip()
        extract_ans_temp = ans.split('ки')[0]
        extract_ans_temp = extract_ans_temp.strip()
        # if len(extract_ans_temp)>0 and extract_ans_temp[-1] == 'ки':
        #     extract_ans = extract_ans_temp[0:-1]
        # else:
        extract_ans = extract_ans_temp
        # extract_ans = extract_ans.strip()
        if util.is_equiv(extract_ans, answer.lower().strip()):
            return True
        else:
            return False
    else:
...
def test_hendrycks_math(model, start=0, end=MAX_INT, batch_size=1, tensor_parallel_size=1):
    hendrycks_math_ins = []
    ...
    ds = load_dataset('lighteval/MATH', 'all')['test'] # .select(range(20))
    ...
    # print(hendrycks_math_answers[:20])
    ...
    for idx, (prompt, prompt_answer) in enumerate(zip(batch_hendrycks_math_ins, hendrycks_math_answers)):
        ...
        # print('completions====', completions[:20])
...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default='HanningZhang/Mistral-MetaMath-7b')  # model path
...
if __name__ == "__main__":
    args = parse_args()
    test_hendrycks_math(model=args.model, start=args.start, end=args.end, batch_size=args.batch_size, tensor_parallel_size=args.tensor_parallel_size)